...
import { electricalItemService } from '../../services';
import type { ElectricalItem, CreateElectricalItemDto } from '../../types';
...
export function ElectricalItemsPage() {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<ElectricalItem | null>(null);
    const [page, setPage] = useState(1);
    const queryClient = useQueryClient();

    const { data, isLoading } = useQuery({ queryKey: ['electrical-items', page], queryFn: () => electricalItemService.getAll({ page, limit: 10 }) });

    const createMutation = useMutation({
        mutationFn: (d: CreateElectricalItemDto) => electricalItemService.create(d),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['electrical-items'] }); toast.success('Electrical item created'); closeModal(); },
        onError: () => toast.error('Failed to create electrical item'),
    });
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: CreateElectricalItemDto }) => electricalItemService.update(id, data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['electrical-items'] }); toast.success('Electrical item updated'); closeModal(); },
        onError: () => toast.error('Failed to update electrical item'),
    });
    const deleteMutation = useMutation({
        mutationFn: (id: string) => electricalItemService.delete(id),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['electrical-items'] }); toast.success('Electrical item deleted'); },
    ...
    const onSubmit = (d: ElectricalItemFormData) => editingItem ? updateMutation.mutate({ id: editingItem.id, data: d }) : createMutation.mutate(d);

    // @ts-expect-error Backend returns data.data instead of data.items for this endpoint
    const items: ElectricalItem[] = data?.items || data?.data || (Array.isArray(data) ? data : []);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
                <div>
                    <h1 style={{ margin: '0 0 4px', fontSize: '22px', fontWeight: '800', color: '#0f172a', letterSpacing: '-0.3px' }}>Electrical</h1>
                    <p style={{ margin: 0, fontSize: '13.5px', color: '#64748b' }}>Manage electrical items (priced per system size)</p>
                </div>
                ...
                >
                    <Plus style={{ width: '15px', height: '15px' }} /> Add Electrical Item
                </button>
            </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #f1f5f9', borderRadius: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #f8fafc', display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <span style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a' }}>All Electrical Items</span>
                ...
                {isLoading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                        <Plug style={{ width: '40px', height: '40px', color: '#e2e8f0', margin: '0 auto 10px' }} />
                        <p style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: '700', color: '#374151' }}>No electrical items yet</p>
                        <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8' }}>Add your first electrical item to get started</p>
                    </div>
                ) : (
                                    ...
                                    >
                                        ...
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#fdf4ff', border: '1px solid #f0abfc', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                    <Plug style={{ width: '16px', height: '16px', color: '#a21caf' }} />
                                                </div>
                                                <div>
                                        ...
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                ...
                                                <IconBtn title="Delete" danger onClick={() => { if (confirm('Delete this electrical item?')) deleteMutation.mutate(item.id); }}>
            ...
            {/* ── Modal ── */}
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingItem ? 'Edit Electrical Item' : 'Add Electrical Item'}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <div>
                            ...
                            <FocusInput placeholder="Enter electrical item name" {...register('name')} />
