...
const capacityCfg: BadgeCfg = { bg: '#f0fdf4', color: '#16a34a', border: '#bbf7d0' };
...
export function InvertersPage() {
    ...
    const { data, isLoading } = useQuery({ queryKey: ['inverters', page], queryFn: () => inverterService.getAll({ page, limit: 10 }) });
    const { data: brands } = useQuery({ queryKey: ['brands', 'inverter'], queryFn: () => brandService.getByType(BrandType.INVERTER) });

    const createMutation = useMutation({
        mutationFn: (d: CreateInverterDto) => inverterService.create(d),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['inverters'] }); toast.success('Inverter created'); closeModal(); },
        onError: () => toast.error('Failed to create inverter'),
    });
    const updateMutation = useMutation({
        mutationFn: ({ id, data }: { id: string; data: CreateInverterDto }) => inverterService.update(id, data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['inverters'] }); toast.success('Inverter updated'); closeModal(); },
        onError: () => toast.error('Failed to update inverter'),
    });
    const deleteMutation = useMutation({
        mutationFn: (id: string) => inverterService.delete(id),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['inverters'] }); toast.success('Inverter deleted'); },
        onError: () => toast.error('Failed to delete inverter'),
    });

    const { register, handleSubmit, reset, formState: { errors } } = useForm<InverterFormValues, unknown, InverterFormData>({ resolver: zodResolver(inverterSchema) });

    const openCreateModal = () => { setEditingInverter(null); reset({ model: '', price: 0, capacity: '', description: '', brandId: '', isActive: true }); setIsModalOpen(true); };
    const openEditModal = (inv: Inverter) => { setEditingInverter(inv); reset({ model: inv.model, price: inv.price, capacity: inv.capacity || '', description: inv.description || '', brandId: inv.brandId, isActive: inv.isActive }); setIsModalOpen(true); };
    const closeModal = () => { setIsModalOpen(false); setEditingInverter(null); reset(); };
    const onSubmit = (d: InverterFormData) => editingInverter ? updateMutation.mutate({ id: editingInverter.id, data: d }) : createMutation.mutate(d);

    // @ts-expect-error Backend returns data.data instead of data.items for this endpoint
    const inverters: Inverter[] = data?.items || data?.data || (Array.isArray(data) ? data : []);
    const totalRecords = (data as any)?.totalCount ?? (data as any)?.total ?? inverters.length;
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
                <div>
                    <h1 style={{ margin: '0 0 4px', fontSize: '22px', fontWeight: '800', color: '#0f172a', letterSpacing: '-0.3px' }}>Inverters</h1>
                    <p style={{ margin: 0, fontSize: '13.5px', color: '#64748b' }}>Manage your inverter products</p>
                </div>
                ...
                >
                    <Plus style={{ width: '15px', height: '15px' }} /> Add Inverter
                </button>
            </div>
            ...
            <div style={{ background: '#fff', border: '1px solid #f1f5f9', borderRadius: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <span style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a' }}>All Inverters</span>
                        <Pagination
                            currentPage={page}
                ...
                {isLoading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                        <Zap style={{ width: '40px', height: '40px', color: '#e2e8f0', margin: '0 auto 10px' }} />
                        <p style={{ margin: '0 0 4px', fontSize: '15px', fontWeight: '700', color: '#374151' }}>No inverters yet</p>
                        <p style={{ margin: 0, fontSize: '13px', color: '#94a3b8' }}>Add your first inverter to get started</p>
                    </div>
                ) : (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
                            <thead>
                                <tr style={{ background: '#f8fafc' }}>
                                    {['Model', 'Brand', 'Capacity', 'Price', 'Status', 'Actions'].map(h => (
                            ...
                            <tbody>
                                {inverters.map((inv, idx) => (
                                    <tr key={inv.id}
                                        style={{ borderBottom: idx < inverters.length - 1 ? '1px solid #f8fafc' : 'none', transition: 'background 0.12s' }}
                                        ...
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: '#f0fdf4', border: '1px solid #bbf7d0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                    <Zap style={{ width: '16px', height: '16px', color: '#16a34a' }} />
                                                </div>
                                                <span style={{ fontSize: '13.5px', fontWeight: '700', color: '#0f172a' }}>{inv.model}</span>
                                            </div>
                                        </td>
                                        {/* Brand */}
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <span style={{ fontSize: '13.5px', color: '#374151', fontWeight: '500' }}>{inv.brand?.name || '—'}</span>
                                        </td>
                                        {/* Capacity */}
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            {inv.capacity
                                                ? <Pill cfg={capacityCfg} label={inv.capacity} />
                                                : <span style={{ fontSize: '13px', color: '#94a3b8' }}>—</span>
                                            }
                                        </td>
                                        {/* Price */}
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <span style={{ fontSize: '13.5px', fontWeight: '800', color: '#0f172a' }}>Rs. {Number(inv.price).toLocaleString()}</span>
                                        </td>
                                        {/* Status */}
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <Pill cfg={inv.isActive ? activeCfg : inactiveCfg} label={inv.isActive ? 'Active' : 'Inactive'} />
                                        </td>
                                        {/* Actions */}
                                        <td style={{ padding: '13px 18px', verticalAlign: 'middle' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                                <IconBtn title="Edit" onClick={() => openEditModal(inv)}>
                                                    <Pencil style={{ width: '14px', height: '14px' }} />
                                                </IconBtn>
                                                <IconBtn title="Delete" danger onClick={() => { if (confirm('Delete this inverter?')) deleteMutation.mutate(inv.id); }}>
            ...
            {/* Modal */}
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingInverter ? 'Edit Inverter' : 'Add Inverter'}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                            <div>
                                ...
                                <FocusInput placeholder="e.g., Hybrid 10kW" {...register('model')} />
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                            ...
                            <div>
                                ...
                                <FocusInput placeholder="e.g., 10kW" {...register('capacity')} />
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px', borderRadius: '10px', background: '#f8fafc', border: '1px solid #f1f5f9', cursor: 'pointer' }}>
                            ...
                            <div>
                                <p style={{ margin: 0, fontSize: '13px', fontWeight: '600', color: '#0f172a' }}>Active Inverter</p>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '8px', paddingTop: '8px', borderTop: '1px solid #f1f5f9', marginTop: '4px' }}>
                            ...
                            <button type="submit" disabled={createMutation.isPending || updateMutation.isPending} style={{
                                ...
                                onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 2px 10px rgba(37,99,235,0.28)'; }}
                            >
                                {editingInverter ? 'Update Inverter' : 'Create Inverter'}
                            </button>
                        </div>
                    </div>
                </form>
